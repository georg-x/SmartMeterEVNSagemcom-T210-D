...
thingSpeakApiKey = config['thingSpeakApiKey']

#Tasmota
useTasmota = config['useTasmota']
tasmotaIpAdresse = config['tasmotaIpAdresse']
tasmotaUser = config['tasmotaUser']
tasmotaPasswort = config ['tasmotaPasswort']
tasmotaEinspeiseMin = config ['tasmotaEinspeiseMin']
# Variable zur Verwendung bei Tasmota
oldueberschuss = 'undef'
...
#        print(found_lines)
    ...
    if useTasmota:
        ueberschuss = 'undef'
        if ( (MomentanleistungP - MomentanleistungN) < ( - tasmotaEinspeiseMin ) ):
            ueberschuss = 'On'
        if ( (MomentanleistungP - MomentanleistungN) >0 ):
            ueberschuss = 'Off'
        if ( (ueberschuss != 'undef') and ( oldueberschuss != ueberschuss )):
           print("-----")
           print(str(ueberschuss))
           print(str(oldueberschuss))
           print("-----")
